	...
	public void setDetails(String result, ITestResult testResult) throws NumberFormatException, Exception {
		...
		try {
			splunkReport.reporting.put("testStatus", classHelper.testResults
					.get(PerfectoRunner.availableReportOptions.scriptStatus.toString()).toString());
		} catch (Exception ex) {
		...
		try {
			splunkReport.reporting.put("model",
					classHelper.testResults.get(PerfectoRunner.availableReportOptions.model.toString()).toString());
			splunkReport.reporting.put("os",
					classHelper.testResults.get(PerfectoRunner.availableReportOptions.os.toString()).toString());
			splunkReport.reporting.put("osVersion",
					classHelper.testResults.get(PerfectoRunner.availableReportOptions.osVersion.toString()).toString());
			splunkReport.reporting.put("device",
					classHelper.testResults.get(PerfectoRunner.availableReportOptions.deviceId.toString()).toString());
		} catch (Exception ex) {
		...
		try {
			splunkReport.reporting.put("testResults",classHelper.testResults
					.get(PerfectoRunner.availableReportOptions.scriptName.toString()).toString());
			splunkReport.reporting.put("reportId",classHelper.reportId = classHelper.testResults
					.get(PerfectoRunner.availableReportOptions.reportId.toString()).toString());
					splunkReport.reporting.put("executionId",classHelper.executionId = classHelper.testResults
					.get(PerfectoRunner.availableReportOptions.executionId.toString()).toString());
		} catch (Exception ex) {

		}

		try {
			Map<String, String> transactions = (Map<String, String>) classHelper.testResults
					.get(PerfectoRunner.availableReportOptions.transactions.toString());

			int i = 1;
			for (String key : transactions.keySet()) {
				splunkReport.startTransaction("Step_" + i, key.toString());
				splunkReport.endTransaction("Step_" + i, Long.parseLong(transactions.get(key).toString().trim()));
				i++;
			}
		} catch (Exception ex) {
		...
		classHelper.perfectoReport= classHelper.testResults
				.get(PerfectoRunner.availableReportOptions.reportUrl.toString()).toString();
